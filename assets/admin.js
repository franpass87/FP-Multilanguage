...
(function () {
    ...
    const testResult = document.getElementById('fpml-test-result');

    if (testApiKeyButton && testResult) {
        testApiKeyButton.addEventListener('click', async () => {
            const endpoint = testApiKeyButton.getAttribute('data-endpoint') || '/wp-json/fpml/v1/test-provider';
            let nonce = testApiKeyButton.getAttribute('data-nonce') || document.querySelector('[data-nonce]')?.getAttribute('data-nonce') || '';

            const apiKeyInput = document.getElementById('fpml-openai-api-key');
            if (!apiKeyInput || !apiKeyInput.value) {
                testResult.innerHTML = '<div class="notice notice-error inline" style="padding: 12px; margin-top: 10px;"><p>Inserisci prima una API key.</p></div>';
                return;
            }

            testApiKeyButton.disabled = true;
            testApiKeyButton.textContent = '⏳ Testing...';
            testResult.innerHTML = '<div class="notice notice-info inline" style="padding: 12px; margin-top: 10px;"><p>⏳ Test connessione in corso...</p></div>';
            ...
            try {
                ...
                if (!response.ok || !payload || payload.success !== true) {
                    const message = (payload && payload.message) || (payload && payload.data && payload.data.message) || 'Test connessione fallito.';
                    testResult.innerHTML = '<div class="notice notice-error inline" style="padding: 12px; margin-top: 10px;"><p><strong>❌ Errore:</strong> ' + escapeHtml(message) + '</p></div>';
                    return;
                }

                const provider = payload.provider || 'OpenAI';
                const elapsed = payload.elapsed || 0;
                const characters = payload.characters || 0;
                const estimatedCost = payload.estimated_cost || 0;
                const translation = payload.translation || '';

                let html = '<div class="notice notice-success inline" style="padding: 12px; margin-top: 10px;">';
                html += '<p><strong>✅ Connessione OK!</strong></p>';
                html += '<p style="margin: 8px 0 0 0;">';
                html += '<strong>Provider:</strong> ' + escapeHtml(provider) + '<br>';
                html += '<strong>Tempo risposta:</strong> ' + elapsed.toFixed(2) + 's<br>';
                html += '<strong>Caratteri testati:</strong> ' + characters.toLocaleString() + '<br>';
                html += '<strong>Costo stimato:</strong> €' + estimatedCost.toFixed(6) + '<br>';
                if (translation) {
                    html += '<strong>Traduzione di prova:</strong> "' + escapeHtml(translation) + '"';
                }
                html += '</p>';
                html += '</div>';
                ...
                testResult.innerHTML = '<div class="notice notice-error inline" style="padding: 12px; margin-top: 10px;"><p><strong>❌ Errore:</strong> ' + escapeHtml(error && error.message ? error.message : 'Errore sconosciuto') + '</p></div>';
            } finally {
                testApiKeyButton.disabled = false;
                testApiKeyButton.textContent = '🧪 Test Connessione';
            }
        });
    }
