...
class JobEnqueuer {
	 ...
	 * @param \FPML_Settings|null $settings Optional settings instance for DI.
	 */
	public function __construct( $queue = null, $settings = null ) {
	...
	public static function instance() {
		_doing_it_wrong( 
			'FP\Multilanguage\Translation\JobEnqueuer::instance()', 
			'JobEnqueuer::instance() is deprecated. Use dependency injection via container instead.', 
			'1.0.0' 
		);
...
public function enqueue_post_jobs( $source_post, $target_post, $update ) {
	...
	foreach ( $fields as $field ) {
		$value = $this->get_post_field_value( $source_post, $field );
		$hash = $this->hash_value( $value );

		if ( ! $hash ) {
			\FP\Multilanguage\Logger::debug( 'Skipping field (no hash)', array( 'field' => $field ) );
			continue;
		}

		$job_id = $this->queue->enqueue( 'post', $source_post->ID, $field, $hash );
		$this->update_post_status_flag( $target_post->ID, $field, 'needs_update' );
		$enqueued = true;
		
		\FP\Multilanguage\Logger::debug( 'Field enqueued', array(
			'field' => $field,
			'job_id' => $job_id,
			'hash' => $hash,
		) );
		...
		if ( $enqueued ) {
			update_post_meta( $target_post->ID, '_fpml_translation_status', 'pending' );
		}
