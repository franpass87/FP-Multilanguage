 ...
 *
 * @since 0.2.0
 * @since 1.0.0 Supports dependency injection while maintaining singleton for backward compatibility
 */
class Settings {
/**
 * Option key.
...
/**
 * Retrieve singleton (for backward compatibility).
 *
 * @since 0.2.0
...
public static function instance() {
	_doing_it_wrong( 
		'FP\Multilanguage\Settings::instance()', 
		'Settings::instance() is deprecated. Use dependency injection via container instead.', 
		'1.0.0' 
	);
 ...
 * @return void
 */
public function invalidate_cache( $old_value, $value, $option ) {
	// Ricarica dal database per assicurarsi che i valori siano completi e sanitizzati
	$saved_settings = get_option( self::OPTION_KEY, array() );
	$defaults = $this->get_defaults();
...
public function get_defaults() {
		...
		'slug_redirect'           => true,
			...
			'manual_translation_mode'   => true, // Disabilita batch automatici, traduzioni solo on-demand
			'auto_translate_strings'    => true, // Traduci automaticamente stringhe hardcoded
			'log_level'                 => defined( 'WP_DEBUG' ) && WP_DEBUG ? 'debug' : 'info', // Livello logging
...
// Check if enabled_languages changed
...
// Flush rewrite rules if routing mode or enabled languages changed
...
$data['excluded_shortcodes']     = sanitize_textarea_field( $data['excluded_shortcodes'] );
	...
	$data['manual_translation_mode']   = ! empty( $data['manual_translation_mode'] ); // Disabilita batch automatici
	$data['auto_translate_strings']    = ! empty( $data['auto_translate_strings'] ); // Traduci automaticamente stringhe hardcoded
