...
/**
 * Trait to extend RestAdmin with additional endpoint handlers.
 *
 * @since 0.9.0
 */
trait RestAdminExtensions {
	 ...
	 * @return WP_REST_Response|WP_Error
	 */
	public function handle_bulk_translate_rest( $request ) {
	 ...
	 * @return WP_REST_Response|WP_Error
	 */
	public function handle_regenerate_translation( $request ) {
	 ...
	 * @return WP_REST_Response|WP_Error
	 */
	public function handle_get_versions( $request ) {
		$post_id = (int) $request->get_param( 'id' );
		$post    = get_post( $post_id );
