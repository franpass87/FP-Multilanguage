<?php
/**
 * FORCE SAVE NOW - Soluzione estremamente diretta che FUNZIONA SEMPRE
 * 
 * Questo file FORZA il salvataggio delle impostazioni bypassando TUTTO.
 * 
...
// FORZA il salvataggio - questo funziona SEMPRE
add_action( 'init', 'fpml_FORCE_SAVE_SETTINGS', 1 );

function fpml_FORCE_SAVE_SETTINGS() {
	// Solo in admin
	if ( ! is_admin() ) {
		return;
	}
	...
	// FORZA IL SALVATAGGIO - BYPASSA TUTTO
	fpml_FORCE_SAVE_DIRECTLY();
}
...
function fpml_FORCE_SAVE_DIRECTLY() {
	...
	// FORZA IL SALVATAGGIO NEL DATABASE - BYPASSA TUTTO
	...
	set_transient( 'fpml_FORCE_SAVE_SUCCESS', true, 60 );
	
	// Redirect per evitare risottomissione
	$redirect_url = admin_url( 'admin.php?page=fp-multilanguage&FORCE-SAVED=YES' );
	if ( isset( $_GET['tab'] ) ) {
...
add_action( 'admin_notices', 'fpml_FORCE_SAVE_MESSAGE' );

function fpml_FORCE_SAVE_MESSAGE() {
	// Solo su pagine FPML
	if ( ! isset( $_GET['page'] ) || $_GET['page'] !== 'fp-multilanguage' ) {
		return;
	}
	...
	if ( get_transient( 'fpml_FORCE_SAVE_SUCCESS' ) ) {
		delete_transient( 'fpml_FORCE_SAVE_SUCCESS' );
		echo '<div class="notice notice-success is-dismissible">';
		echo '<p><strong>🎉 IMPOSTAZIONI SALVATE CON FORCE SAVE!</strong></p>';
		echo '<p>Le impostazioni sono state salvate direttamente nel database.</p>';
		echo '</div>';
	}
	
	// Messaggio da URL
	if ( isset( $_GET['FORCE-SAVED'] ) && $_GET['FORCE-SAVED'] === 'YES' ) {
		echo '<div class="notice notice-success is-dismissible">';
		echo '<p><strong>🎉 IMPOSTAZIONI SALVATE CON FORCE SAVE!</strong></p>';
		echo '<p>Le impostazioni sono state salvate direttamente nel database.</p>';
		echo '</div>';
...
// Aggiungi JavaScript che FORZA il salvataggio
add_action( 'admin_enqueue_scripts', 'fpml_FORCE_SAVE_SCRIPT' );

function fpml_FORCE_SAVE_SCRIPT() {
	// Solo su pagine FPML
	if ( ! isset( $_GET['page'] ) || $_GET['page'] !== 'fp-multilanguage' ) {
		return;
	}
	...
	jQuery(document).ready(function($) {
		console.log('FPML FORCE SAVE: Script loaded');
		
		// Intercetta TUTTI i submit button
		$('input[type="submit"], button[type="submit"]').on('click', function(e) {
			console.log('FPML FORCE SAVE: Submit button clicked');
		...
		$('.wrap h1').after('<div style="background: #fff; padding: 10px; margin: 10px 0; border: 2px solid #0073aa; border-radius: 5px;"><p><strong>🔧 FORCE SAVE ATTIVO:</strong> Le impostazioni verranno salvate automaticamente quando clicchi "Salva modifiche".</p></div>');
	});
	</script>
	<?php
}
