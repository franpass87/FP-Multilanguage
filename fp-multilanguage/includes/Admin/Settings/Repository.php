...
class Repository {
	...
	public const MANUAL_STRINGS_FALLBACK_OPTION = 'fp_multilanguage_strings';

	private const CACHE_GROUP               = 'fp_multilanguage_settings';
	private const CACHE_KEY_OPTIONS         = 'options';
	private const CACHE_KEY_MANUAL_STRINGS  = 'manual_strings';
	private const CACHE_KEY_MANUAL_METADATA = 'manual_strings_metadata';
	...
	private ?array $cachedOptions = null;
		 ...
		 */
	private ?array $cachedManualStrings = null;

		/**
		 * @var array<string, array{context:string,original:string,updated:string}>|null
		 */
	private ?array $cachedManualStringMetadata = null;
	...
	public function register_cache_hooks(): void {
		...
		if ( function_exists( 'add_action' ) ) {
				...
				add_action( 'update_option_' . self::MANUAL_STRINGS_OPTION, array( $this, 'clear_manual_strings_cache' ), 10, 0 );
				add_action( 'add_option_' . self::MANUAL_STRINGS_OPTION, array( $this, 'clear_manual_strings_cache' ), 10, 0 );
				add_action( 'delete_option_' . self::MANUAL_STRINGS_OPTION, array( $this, 'clear_manual_strings_cache' ), 10, 0 );
				add_action( 'update_option_' . self::MANUAL_STRINGS_FALLBACK_OPTION, array( $this, 'clear_manual_strings_metadata_cache' ), 10, 0 );
				add_action( 'add_option_' . self::MANUAL_STRINGS_FALLBACK_OPTION, array( $this, 'clear_manual_strings_metadata_cache' ), 10, 0 );
				add_action( 'delete_option_' . self::MANUAL_STRINGS_FALLBACK_OPTION, array( $this, 'clear_manual_strings_metadata_cache' ), 10, 0 );
		}
	...
	public function get_options(): array {
		if ( null === $this->cachedOptions ) {
			if ( function_exists( 'wp_cache_get' ) ) {
						$cached = wp_cache_get( self::CACHE_KEY_OPTIONS, self::CACHE_GROUP );
				if ( is_array( $cached ) ) {
					$this->cachedOptions = $cached;
				}
			}
			...
			if ( null === $this->cachedOptions ) {
				if ( ! function_exists( 'get_option' ) ) {
						...
						$this->cachedOptions = wp_parse_args( is_array( $optionsRaw ) ? $optionsRaw : array(), self::DEFAULTS );
							...
							$this->persist_options_cache( $this->cachedOptions );
			}
		}
	...
	public function clear_cache(): void {
		...
		if ( function_exists( 'wp_cache_delete' ) ) {
				wp_cache_delete( self::CACHE_KEY_OPTIONS, self::CACHE_GROUP );
		}
	}

	public function get_manual_strings(): array {
		if ( null === $this->cachedManualStrings && function_exists( 'wp_cache_get' ) ) {
				$cached = wp_cache_get( self::CACHE_KEY_MANUAL_STRINGS, self::CACHE_GROUP );
			if ( is_array( $cached ) ) {
				$this->cachedManualStrings = $cached;
			}
		}

		if ( null === $this->cachedManualStrings ) {
				$stored                    = get_option( self::MANUAL_STRINGS_OPTION, array() );
				$this->cachedManualStrings = is_array( $stored ) ? $stored : array();
				$this->persist_manual_strings_cache( $this->cachedManualStrings );
		}

			return $this->cachedManualStrings;
	}
	...
	public function update_manual_string( string $key, string $language, string $value ): void {
		...
		if ( '' === $value ) {
			...
			$strings[ $key ]           = $translations;
				...
				$this->set_cached_manual_strings( $strings );

				$this->sync_manual_string_storage( $key, $strings[ $key ] ?? array() );

				$this->clear_manual_strings_metadata_cache();
	...
	private function set_cached_options( array $options ): void {
			...
			$this->persist_options_cache( $options );
	}

	private function set_cached_manual_strings( array $strings ): void {
			$this->cachedManualStrings = $strings;
			$this->persist_manual_strings_cache( $strings );
	}

	private function persist_manual_strings_cache( array $strings ): void {
		if ( function_exists( 'wp_cache_set' ) ) {
				wp_cache_set( self::CACHE_KEY_MANUAL_STRINGS, $strings, self::CACHE_GROUP );
		}
	}

	private function persist_options_cache( array $options ): void {
		if ( function_exists( 'wp_cache_set' ) ) {
				wp_cache_set( self::CACHE_KEY_OPTIONS, $options, self::CACHE_GROUP );
		}
	}
	...
	private function sync_manual_string_table( string $key, array $translations ): void {
		...
		if ( is_array( $existing ) ) {
			...
			$original = isset( $existing['original'] ) ? (string) $existing['original'] : '';
				...
				$fallback = get_option( self::MANUAL_STRINGS_FALLBACK_OPTION, array() );
	...
	private function sync_manual_string_fallback( string $key, array $translations ): void {
				$option = get_option( self::MANUAL_STRINGS_FALLBACK_OPTION, array() );
		if ( ! is_array( $option ) ) {
			$option = array();
		}
		...
		if ( empty( $translations ) ) {
			...
			$option[ $key ]        = $entry;
				...
				update_option( self::MANUAL_STRINGS_FALLBACK_OPTION, $option );
	}
	...
	private function get_manual_string_metadata(): array {
		if ( null !== $this->cachedManualStringMetadata ) {
				return $this->cachedManualStringMetadata;
		}

		if ( function_exists( 'wp_cache_get' ) ) {
				$cached = wp_cache_get( self::CACHE_KEY_MANUAL_METADATA, self::CACHE_GROUP );
			if ( is_array( $cached ) ) {
					$this->cachedManualStringMetadata = $cached;
		...
		if ( is_array( $fallback ) ) {
			...
			$this->persist_manual_strings_metadata_cache( $metadata );

			return $metadata;
	}

	public function clear_manual_strings_cache(): void {
			$this->cachedManualStrings = null;
		if ( function_exists( 'wp_cache_delete' ) ) {
				wp_cache_delete( self::CACHE_KEY_MANUAL_STRINGS, self::CACHE_GROUP );
		}
	}

	public function clear_manual_strings_metadata_cache(): void {
			$this->cachedManualStringMetadata = null;
		if ( function_exists( 'wp_cache_delete' ) ) {
				wp_cache_delete( self::CACHE_KEY_MANUAL_METADATA, self::CACHE_GROUP );
		}
	}

	private function persist_manual_strings_metadata_cache( array $metadata ): void {
			$this->cachedManualStringMetadata = $metadata;
		if ( function_exists( 'wp_cache_set' ) ) {
				wp_cache_set( self::CACHE_KEY_MANUAL_METADATA, $metadata, self::CACHE_GROUP );
		}
	}
}
