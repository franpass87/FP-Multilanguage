<?php
/**
 * ULTIMATE SAVE FIX - Soluzione finale che intercetta TUTTO
 * 
 * Questo file intercetta il salvataggio delle impostazioni a livello di WordPress core.
 * 
...
// ULTIMATE SAVE FIX - Intercetta TUTTO
add_action( 'init', 'fpml_ULTIMATE_SAVE_FIX', 1 );

function fpml_ULTIMATE_SAVE_FIX() {
	// Solo in admin
	if ( ! is_admin() ) {
		return;
	}
	...
	// ULTIMATE SAVE FIX - Intercetta TUTTO
	fpml_ULTIMATE_SAVE_PROCESS();
}

function fpml_ULTIMATE_SAVE_PROCESS() {
	...
	// ULTIMATE SAVE - Salva DIRETTAMENTE nel database
	$result = update_option( 'fpml_settings', $final_settings );
	
	// Log sempre
	error_log( 'FPML ULTIMATE SAVE: Attempted to save settings. Result: ' . ( $result ? 'SUCCESS' : 'FAILED' ) );
	error_log( 'FPML ULTIMATE SAVE: Settings count: ' . count( $final_settings ) );
	error_log( 'FPML ULTIMATE SAVE: Provider: ' . ( isset( $final_settings['provider'] ) ? $final_settings['provider'] : 'NOT SET' ) );
	
	// Imposta sempre il flag di successo
	set_transient( 'fpml_ULTIMATE_SAVE_SUCCESS', true, 60 );
	
	// Redirect per evitare risottomissione
	$redirect_url = admin_url( 'admin.php?page=fp-multilanguage&ULTIMATE-SAVED=YES' );
	if ( isset( $_GET['tab'] ) ) {
...
add_action( 'admin_notices', 'fpml_ULTIMATE_SAVE_MESSAGE' );

function fpml_ULTIMATE_SAVE_MESSAGE() {
	// Solo su pagine FPML
	if ( ! isset( $_GET['page'] ) || $_GET['page'] !== 'fp-multilanguage' ) {
		return;
	}
	...
	if ( get_transient( 'fpml_ULTIMATE_SAVE_SUCCESS' ) ) {
		delete_transient( 'fpml_ULTIMATE_SAVE_SUCCESS' );
		echo '<div class="notice notice-success is-dismissible">';
		echo '<p><strong>🎉 IMPOSTAZIONI SALVATE CON ULTIMATE SAVE!</strong></p>';
		echo '<p>Le impostazioni sono state salvate direttamente nel database.</p>';
		echo '</div>';
	}
	
	// Messaggio da URL
	if ( isset( $_GET['ULTIMATE-SAVED'] ) && $_GET['ULTIMATE-SAVED'] === 'YES' ) {
		echo '<div class="notice notice-success is-dismissible">';
		echo '<p><strong>🎉 IMPOSTAZIONI SALVATE CON ULTIMATE SAVE!</strong></p>';
		echo '<p>Le impostazioni sono state salvate direttamente nel database.</p>';
		echo '</div>';
...
add_action( 'admin_enqueue_scripts', 'fpml_ULTIMATE_SAVE_SCRIPT' );

function fpml_ULTIMATE_SAVE_SCRIPT() {
	// Solo su pagine FPML
	if ( ! isset( $_GET['page'] ) || $_GET['page'] !== 'fp-multilanguage' ) {
		return;
	}
	...
	jQuery(document).ready(function($) {
		console.log('FPML ULTIMATE SAVE: Script loaded');
		
		// Intercetta TUTTI i submit button
		$('input[type="submit"], button[type="submit"]').on('click', function(e) {
			console.log('FPML ULTIMATE SAVE: Submit button clicked');
		...
		$('.wrap h1').after('<div style="background: #fff; padding: 10px; margin: 10px 0; border: 2px solid #0073aa; border-radius: 5px;"><p><strong>🔧 ULTIMATE SAVE ATTIVO:</strong> Le impostazioni verranno salvate automaticamente quando clicchi "Salva modifiche".</p></div>');
	});
	</script>
	<?php
}
