...
try {
	if ( class_exists( '\FPML_Plugin_Detector' ) || class_exists( '\FP\Multilanguage\PluginDetector' ) ) {
		$detector_class = class_exists( '\FPML_Plugin_Detector' ) ? '\FPML_Plugin_Detector' : '\FP\Multilanguage\PluginDetector';
		$detector = $detector_class::instance();
		
		if ( $detector && method_exists( $detector, 'get_detected_plugins' ) ) {
			$detected = $detector->get_detected_plugins();
		}
		
		if ( $detector && method_exists( $detector, 'get_detection_summary' ) ) {
	...
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
		error_log( 'FP Multilanguage Compatibility Tab Error: ' . $e->getMessage() );
	}
}
