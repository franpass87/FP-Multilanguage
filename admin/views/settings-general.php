...
<button 
	...
	id="fpml-test-api-key" 
	data-endpoint="<?php echo esc_url( rest_url( 'fpml/v1/test-provider' ) ); ?>"
	data-nonce="<?php echo esc_attr( wp_create_nonce( 'wp_rest' ) ); ?>"
	style="margin-left: 10px;">
	🧪 <?php esc_html_e( 'Test Connessione', 'fp-multilanguage' ); ?>
</button>
<button 
	type="button" 
	class="button button-secondary" 
...
<tr>
<th scope="row"><?php esc_html_e( 'Lingue abilitate', 'fp-multilanguage' ); ?></th>
<td>
<?php
$language_manager = \FP\Multilanguage\MultiLanguage\LanguageManager::instance();
$available_languages = $language_manager->get_all_languages();
$enabled_languages = isset( $options['enabled_languages'] ) && is_array( $options['enabled_languages'] ) ? $options['enabled_languages'] : array( 'en' );
?>
<fieldset>
<?php foreach ( $available_languages as $lang_code => $lang_info ) : ?>
	<label style="display: block; margin-bottom: 8px;">
		<input 
			type="checkbox" 
			name="<?php echo esc_attr( FPML_Settings::OPTION_KEY ); ?>[enabled_languages][]" 
			value="<?php echo esc_attr( $lang_code ); ?>" 
			<?php checked( in_array( $lang_code, $enabled_languages, true ) ); ?> 
		/>
		<span style="font-size: 20px; margin-right: 8px;"><?php echo esc_html( $lang_info['flag'] ); ?></span>
		<strong><?php echo esc_html( $lang_info['name'] ); ?></strong>
		<span style="color: #666; margin-left: 8px;">(<?php echo esc_html( $lang_code ); ?>)</span>
	</label>
<?php endforeach; ?>
</fieldset>
<p class="fpml-field-description">
...
<?php esc_html_e( 'Segmento /en/, /de/, /fr/, /es/', 'fp-multilanguage' ); ?>
</label>
<br />
<label>
...
<p class="fpml-field-description"><?php esc_html_e( 'Scegli il meccanismo preferito per le lingue target. Il routing a segmento crea URL tipo /en/, /de/, etc.', 'fp-multilanguage' ); ?></p>
</td>
</tr>
<tr>
...
<!-- Sezione Traduzione Parti del Sito -->
<div class="fpml-site-translation-section" style="margin-top: 40px; padding: 20px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;">
	<h2 style="margin-top: 0;">🌐 <?php esc_html_e( 'Traduzione Parti del Sito', 'fp-multilanguage' ); ?></h2>
	<p class="description">
		<?php esc_html_e( 'Traduci menu, widget, opzioni tema e stringhe dei plugin. Le traduzioni saranno visibili quando si seleziona la lingua inglese (bandierina ENG).', 'fp-multilanguage' ); ?>
	</p>
	
	<div class="fpml-translation-actions" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px; margin-top: 20px;">
		<div class="fpml-translation-card" style="padding: 15px; background: #fff; border: 1px solid #ddd; border-radius: 4px;">
			<h3 style="margin-top: 0;">📋 <?php esc_html_e( 'Menu', 'fp-multilanguage' ); ?></h3>
			<p style="font-size: 13px; color: #666;">
				<?php esc_html_e( 'Traduci tutti i menu del sito. I menu tradotti saranno visibili su /en/', 'fp-multilanguage' ); ?>
			</p>
			<button type="button" class="button button-primary fpml-translate-site-part" data-part="menus" style="width: 100%;">
				🚀 <?php esc_html_e( 'Traduci Menu', 'fp-multilanguage' ); ?>
			</button>
			<div class="fpml-translation-status" data-part="menus" style="margin-top: 10px; font-size: 12px; color: #666;"></div>
		</div>
		
		<div class="fpml-translation-card" style="padding: 15px; background: #fff; border: 1px solid #ddd; border-radius: 4px;">
			<h3 style="margin-top: 0;">🧩 <?php esc_html_e( 'Widget', 'fp-multilanguage' ); ?></h3>
			<p style="font-size: 13px; color: #666;">
				<?php esc_html_e( 'Traduci tutti i widget attivi. I widget tradotti saranno visibili su /en/', 'fp-multilanguage' ); ?>
			</p>
			<button type="button" class="button button-primary fpml-translate-site-part" data-part="widgets" style="width: 100%;">
				🚀 <?php esc_html_e( 'Traduci Widget', 'fp-multilanguage' ); ?>
			</button>
			<div class="fpml-translation-status" data-part="widgets" style="margin-top: 10px; font-size: 12px; color: #666;"></div>
		</div>
		
		<div class="fpml-translation-card" style="padding: 15px; background: #fff; border: 1px solid #ddd; border-radius: 4px;">
			<h3 style="margin-top: 0;">🎨 <?php esc_html_e( 'Opzioni Tema', 'fp-multilanguage' ); ?></h3>
			<p style="font-size: 13px; color: #666;">
				<?php esc_html_e( 'Traduci le opzioni del tema Salient. Le traduzioni saranno visibili su /en/', 'fp-multilanguage' ); ?>
			</p>
			<button type="button" class="button button-primary fpml-translate-site-part" data-part="theme-options" style="width: 100%;">
				🚀 <?php esc_html_e( 'Traduci Opzioni Tema', 'fp-multilanguage' ); ?>
			</button>
			<div class="fpml-translation-status" data-part="theme-options" style="margin-top: 10px; font-size: 12px; color: #666;"></div>
		</div>
		
		<div class="fpml-translation-card" style="padding: 15px; background: #fff; border: 1px solid #ddd; border-radius: 4px;">
			<h3 style="margin-top: 0;">🔌 <?php esc_html_e( 'Plugin Comuni', 'fp-multilanguage' ); ?></h3>
			<p style="font-size: 13px; color: #666;">
				<?php esc_html_e( 'Traduci stringhe di WooCommerce, Contact Form 7, e altri plugin comuni.', 'fp-multilanguage' ); ?>
			</p>
			<button type="button" class="button button-primary fpml-translate-site-part" data-part="plugins" style="width: 100%;">
				🚀 <?php esc_html_e( 'Traduci Plugin', 'fp-multilanguage' ); ?>
			</button>
			<div class="fpml-translation-status" data-part="plugins" style="margin-top: 10px; font-size: 12px; color: #666;"></div>
		</div>
	</div>
</div>
...
jQuery(document).ready(function($) {
	$('.fpml-translate-site-part').on('click', function() {
		const $btn = $(this);
		const part = $btn.data('part');
		const $status = $('.fpml-translation-status[data-part="' + part + '"]');
		
		if (!confirm('<?php echo esc_js( __( 'Vuoi tradurre questa parte del sito? La traduzione sarà disponibile su /en/', 'fp-multilanguage' ) ); ?>')) {
			return;
		}
		
		$btn.prop('disabled', true).html('⏳ <?php echo esc_js( __( 'Traduzione in corso...', 'fp-multilanguage' ) ); ?>');
		$status.html('⏳ <?php echo esc_js( __( 'Traduzione in corso...', 'fp-multilanguage' ) ); ?>');
		...
		$.ajax({
			...
			data: {
				...
				part: part,
				_wpnonce: '<?php echo esc_js( wp_create_nonce( 'fpml_translate_site_part' ) ); ?>'
			},
			timeout: 300000,
			success: function(response) {
				if (response.success) {
					$status.html('✅ ' + (response.data.message || '<?php echo esc_js( __( 'Traduzione completata!', 'fp-multilanguage' ) ); ?>'));
					$btn.prop('disabled', false).html('🔄 <?php echo esc_js( __( 'Ritraduci', 'fp-multilanguage' ) ); ?>');
				} else {
					$status.html('❌ ' + (response.data.message || '<?php echo esc_js( __( 'Errore durante la traduzione', 'fp-multilanguage' ) ); ?>'));
					$btn.prop('disabled', false).html('🚀 <?php echo esc_js( __( 'Riprova', 'fp-multilanguage' ) ); ?>');
				}
			},
			error: function(xhr, status, error) {
				$status.html('❌ <?php echo esc_js( __( 'Errore di connessione', 'fp-multilanguage' ) ); ?>');
				$btn.prop('disabled', false).html('🚀 <?php echo esc_js( __( 'Riprova', 'fp-multilanguage' ) ); ?>');
			}
		});
	});
